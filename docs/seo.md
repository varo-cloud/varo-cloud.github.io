...
**全站补齐 SEO 基础设施（meta / canonical / hreflang / robots / sitemap / JSON-LD），首页额外做构建期预渲染（Playwright），在不改部署形态的前提下，让营销首页的首包 HTML 自带可索引内容。**
...
| 全站静态预渲染所有路由 | ❌ 暂不做 | 登录态页、动态模型详情路径多；收益集中在营销入口 |
| **全站 Head 管理 + 首页预渲染** | ✅ | 成本可控，覆盖首页收录与分享预览；其它公开页有 meta / sitemap 兜底 |

优先级：**首页（`/`、`/zh-CN`）> 其它营销页 meta > 用户后台页 noindex。**
...
用户 / 爬虫请求 /
 ...
 GitHub Pages 静态文件
        │
        ├─ index.html          ← 构建期 Playwright 预渲染后的「带正文」HTML
        ├─ zh-CN/index.html    ← 中文首页预渲染
...
vite build
  ...
  → prerender：用 Playwright 打开 / 与 /zh-CN，把渲染结果写回
       dist/index.html
       dist/zh-CN/index.html
```

> 必须先有 `404.html`，再覆盖 `index.html`。否则预渲染服务没有 SPA shell 可回退，深链在预渲染阶段会失败。
...
| 站点地图 | `public/sitemap.xml` | 首页、Models、Pricing、AI Generator、Terms、Privacy（含中英） |
| 隐私页策略 | 账号相关路由 `noindex, nofollow` | 避免后台页进入索引 |

覆盖路由示例：

- 可索引：`home`、`models`、`model-detail`、`pricing`、`ai-generator`、`docs`、`terms`、`privacy`
- 不索引：`auth`、`api-keys`、`billing`、`generations`
...
2. Playwright Chromium 访问 `/`、`/zh-CN`
3. 等待首页挂载标记 `[data-home-ready]`，并确认 Hero 主标题文案出现
4. 再短暂等待 Head 写入后，导出完整 HTML 覆盖产物

固化进 HTML 的内容（SEO 主收益）：

- `<title>`、`<meta description>`、OG / Twitter、canonical、hreflang
- Hero 主标题 / 副标题、各区块**静态营销文案**（Developers、Creators、Value Props、CTA 等）
- 首页 JSON-LD
...
| 其它营销页（Models / Pricing 等） | 仅运行时 Head + sitemap | 有 meta 与入口，但首包未必带完整正文 |
...
- 仅预渲染首页中英两条路由，不是全站 SSG
- 动态接口区块不保证进入静态 HTML
- 模型详情等动态 path 主要靠运行时 Head；首包正文仍偏 CSR
- OG 默认图当前为站点 logo，若要更好分享效果可换专用封面图

**可选增强（按需）**

1. 预渲染等待关键接口完成，或构建时注入模型/Publisher 快照，让动态块也进 HTML  
2. 对 `/models`、`/pricing` 等营销页做同样的轻量预渲染  
3. 模型详情做「热门 slug 列表」批量预渲染  
4. 将 sitemap 改为构建脚本根据公开路由自动生成  
5. Search Console 提交 sitemap，监控覆盖率与索引状态  
...
> 2）构建时用 Playwright 把中英首页预渲染成真正带正文的 HTML。  
>
> 这样搜索引擎和社交平台打开首页源码就能看到品牌与主卖点。  
> 页面里模型列表、价格等仍走接口动态加载——它们丰富用户体验，但不是首页 SEO 的主信号；后续若要长尾词进索引，可以再加强预渲染等待或快照注入。
...
|------|--------|------|
| `/`、`/zh-CN` | ✅ | ✅ |
| `/models`、`/pricing`、`/ai-generator`、法律页等 | ❌（仅 Head + sitemap） | ✅ |
